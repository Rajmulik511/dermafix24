document.addEventListener('DOMContentLoaded', () => {
    ...
    if (slider) {
        ...
        function updateSlider() {
            ...
            cardWidth = (sliderWrapper.offsetWidth / cardsToShow);
            
            slider.style.width = `${totalCards * cardWidth}px`;

            cards.forEach(card => {
                card.style.flex = `0 0 ${cardWidth - cardMargin}px`;
                card.style.margin = `0 ${cardMargin / 2}px`;
            });
        ...
        prevBtn.addEventListener('click', () => {
            if (currentIndex > 0) {
                ...
                currentIndex = totalCards - cardsToShow; // Loop back to end
            }
            updateSlider();
        });
